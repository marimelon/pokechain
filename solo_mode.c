...
static MenuSelect menu(const GameState *state) {
    ...
    while(true) {
        ...
        if(c == KEY_UP || c == KEY_DOWN) {
            cpos = c == KEY_DOWN ? (cpos + 1) % 4 : (cpos + 3) % 4;
        } else if(c == KEY_ENTER || c == '\n') {
            if(cpos == GIVEUP) {
                draw_box(6, 20, 12, 46);
                fill_blank(7, 21, 10, 44);
                mvaddstr(9, 34, "本当に諦めますか？");
                mvaddstr(13, 35, "はい");
                mvaddstr(13, 45, "いいえ");
                mvaddstr(16, 32, "←→：選択 Enter：決定");
                cpos = 1;
...
static void show_result(const GameState *state) {
    ...
    if(state->turn % 2) {
        mvprintw(6, 35, "あなたの勝利です！");
    } else {
        mvprintw(6, 35, "あなたの負けです");
    }
...
void run_sologame(const GameData *gamedata) {
    GameState state = {gamedata, SOLO, 0,     {0},   L"",  L"",
    ...
    for(;;) {
        ...
        if(state.is_show_result) {
            show_result(&state);
            save_plog(&state, SOLO);
            return;
        }
        if(prev_turn != state.turn) {
            next_turn_auto(&state);
        }
    }
}